 ...
 * itself is a thin adapter — it seeds the default graph, builds property sections through the block
...
// Human-readable provenance label for the seeded orb sample, shown in the import block's read-only
// "Source" field. The bytes are bundled with the editor and fetched from its own origin, so this is a
// display label only.
...
export class NodeAssetGraphController {
    ...
    /**
     * Creates a controller seeded with an orb compression graph. ImportGLTF supplies a UV sphere that
     * flows through the KTX2 and Draco transforms (grouped in a "Compression" frame) to ExportGLTF.
    ...
    public constructor(buildClient: INodeAssetBuildClient = new NodeAssetBuildWorkerClient()) {
        ...
        const gltfNode = this._instantiateBlock(importGltfDescriptor, { x: 80, y: 320 });
        const ktx2Node = this._instantiateBlock(ktx2Descriptor, { x: 380, y: 320 });
        const dracoNode = this._instantiateBlock(dracoDescriptor, { x: 620, y: 320 });
        const exportNode = this._instantiateBlock(exportDescriptor, { x: 860, y: 320 });

        this._orbGltfBlock = this._reconciler.getBlock(gltfNode.id)! as ImportGLTFBlock;

        const snapshot: IGraphSnapshot = {
            nodes: [gltfNode, ktx2Node, dracoNode, exportNode],
            wires: [this._createWire(gltfNode, ktx2Node), this._createWire(ktx2Node, dracoNode), this._createWire(dracoNode, exportNode)],
            frames: [
                {
    ...
    /**
     * Loads the bundled orb `.glb` into the seeded ImportGLTF block.
     * @returns A promise that resolves after the asset is loaded.
     */
    public async loadDefaultImportAsync(): Promise<void> {
        const orbGlb = await this._fetchAssetBytesAsync(DefaultSampleAssetUrls.orbGlb);
