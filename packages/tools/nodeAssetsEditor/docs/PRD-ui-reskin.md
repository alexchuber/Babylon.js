...
| **Undo** / **Redo** | **Corrected: the absence of a visible Undo/Redo control is itself the established sibling convention** — all four of NME/NGE/NRGE/NPE deliberately expose undo/redo only via `Ctrl+Z`/`Ctrl+Y` keyboard shortcuts, with zero visible button anywhere in any of them. This is a consistent, 100%-observed pattern, not a gap. | 🔧 **Align: remove the visible Undo/Redo buttons entirely.** A shell toolbar containing only Undo/Redo would itself be a visibly NAE-specific surface, contradicting the reskin goal. Preserve `Ctrl+Z`/`Ctrl+Y` keyboard behavior and the existing history-stack implementation unchanged — only the visible buttons are removed. **Accessibility check performed**: searched this repo's `.github/architecture/accessibility.md` and found no requirement mandating a visible undo/redo control for keyboard-operable actions (that doc covers 3D-scene screen-reader DOM twins, unrelated to editor chrome); no concrete accessibility noncompliance was found that would justify an exception, so none is claimed. If a concrete a11y requirement is identified later, the correct remedy per the sibling family is to add it inside the closest sibling-style command surface (the Properties panel's default view, alongside View/File), not to reinstate a unique top-level toolbar. |
...
**Net effect**: NAE's shell-level toolbar is **removed entirely** — matching "all four siblings have no shell-level toolbar" exactly. Zoom to fit, Reorganize, Save, and Load move into the Properties panel's default (no-selection) view, Fluent-styled, grouped into "View" and "File" sections that mirror sibling placement/grouping semantics. Undo/Redo remain fully functional via keyboard only, matching every sibling exactly, with no visible control anywhere. This is a real behavior change from NAE's current always-visible-toolbar convenience — tracked as its own work item (see Decomposition) since it touches shell registration (`nodeAssetsEditorService.tsx`) and the Properties panel (`PropertiesView.tsx`) directly.
