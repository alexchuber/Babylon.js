...
| Pop-out preview window | `CreatePopup()` child window (universal) | NAE's preview is inline only (`PreviewPane`, `keepMounted: true`) | 🔧 **Optional enhancement**: consider a pop-out affordance for the preview pane matching sibling convention, using the existing `shared-ui-components/popupHelper` (`CreatePopup`) — not a blocking requirement given NAE's preview is compact by design, but include if low-effort. |
