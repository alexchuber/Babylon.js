...
| `Plane.usda`                                    | Single quad mesh on the XZ plane with constant authored normals                                                                                                                                                  | `522122b8bfe845b4fa80bdefb9b2bf108291a5e432879739ce915371bc830923` | 583          | None                                                                                                                                                                     |
...
| `RobotArm2.usda`                                | USDA wrapper with an authored relative prepend reference to the Robot Arm layer                                                                                                                                  | `76e5ffceb2d6eeab82685a4edac57867f4d11bdb77ff9f36528d9c06e6932ad5` | 484          | `RobotArm2/RobotArm.usda`                                                                                                                                                |
| `RobotArm.usda`                                 | USDA wrapper delegating its authored OBJ sidecar through the optional external-asset handler                                                                                                                     | `2271e7e1d3c56fa4641d72f156c9c13fb6d473296a2f7e4bb54a987c1ed814ce` | 611          | `RobotArm/industrial_robot_arm.obj`, `RobotArm/industrial_robot_arm.mtl`, `RobotArm/Robot_Arm_Color.png`                                                                 |
| `RobotArm/industrial_robot_arm.obj`             | Robot Arm geometry sidecar; its `mtllib` names the active MTL below                                                                                                                                              | `e6d2365ed947054f1473784b96d2b20cf68bf79f96cc10c993b900de90a92596` | 5,909,664    | `RobotArm/industrial_robot_arm.mtl`                                                                                                                                      |
| `RobotArm/industrial_robot_arm.mtl`             | Active OBJ material sidecar; its `map_Kd` names the color texture below                                                                                                                                          | `45dbb26ff2c7552b933d13587e4025ff70afc126f4395bd1244d1f5d21ddf11d` | 259          | `RobotArm/Robot_Arm_Color.png`                                                                                                                                           |
| `RobotArm/Robot_Arm_Color.png`                  | OBJ diffuse color texture sidecar; PNG, 4096 x 4096, 8-bit RGB                                                                                                                                                   | `0ea2c54f9d8ac2d1483b0e46208abaf88303c697e840e0c06059f559115268d3` | 12,850,274   | (sidecar of industrial_robot_arm.mtl)                                                                                                                                    |
...
| `seahorse_anim_mtl_variant.usdz`                | Neutral USDZ package with one USDC root, eight archive-local JPEG textures, selected Coral/Idle1 authored variant state, three material subsets, and UsdSkel animation (redacted derivative — see Modifications) | `520e344f0568d442ea7d3b60347d3eff961c1afb101b3e3cd3a61a504d10b285` | 19,466,069   | Embedded entries listed below                                                                                                                                            |
...
## Seahorse USDZ embedded-entry manifest

The committed archive is stored-only and contains exactly these nine normalized entries. The embedded
USDC root hash is over the committed, metadata-scrubbed crate bytes; JPEG hashes and dimensions are
over the committed entry bytes.

| Entry                            | Format | SHA-256                                                            | Size (bytes) |  Resolution |
| -------------------------------- | ------ | ------------------------------------------------------------------ | -----------: | ----------: |
| `seahorse_anim_mtl_variant.usdc` | USDC   | `c15bb309e74c4b9bbf484dc771eb5fc5f34792e7bd2090958d232147066020a1` |   13,022,933 |           — |
| `seahorseeye_n.jpg`              | JPEG   | `21f9d41d79d212f823859e60da9e5a5ef1c6079960447c840be11cc2d41d9c8e` |       17,013 | 1024 × 1024 |
| `seahorse_n.jpg`                 | JPEG   | `cdf9a6090a6da1996226d3903fa22e8472eaaf9e802ba063b0e84616eb13e1e4` |    1,590,807 | 4096 × 4096 |
| `seahorse_o.jpg`                 | JPEG   | `74433a318ba097e7f5627eea85ba01aeb5ec3cab8470f1ea4cd6ab1cb9778e9e` |      573,478 | 4096 × 4096 |
| `seahorseeye_bc.jpg`             | JPEG   | `e564b2ac1a0381eb21d8b7714ad3217ed9df514a7f48cb920bc295d2bf1dffc8` |      133,304 | 1024 × 1024 |
| `seahorseeye_r.jpg`              | JPEG   | `6490a73433957bc11596f82ffb8bc395bd38a9deb399d17b46ea60be424dfe88` |       12,620 | 1024 × 1024 |
| `seahorse_r.jpg`                 | JPEG   | `96fcd25d6c1f9c420ca38cec82326926f4b57353277747825772a359343ef1d4` |    1,459,321 | 4096 × 4096 |
| `seahorse_bc.jpg`                | JPEG   | `a75bd5d76c28cb8c16889e8cb38de8e698c15871b17114189683eaf29025aa0c` |      929,786 | 4096 × 4096 |
| `seahorse_purple_bc.jpg`         | JPEG   | `5a4389f7e1e0cf819f4aba8bcc285e1859511c7c0a56df36ab8c0cdb3e635cc0` |    1,725,779 | 4096 × 4096 |

The supplied pre-scrub package measured 19,494,508 bytes and had SHA-256
`f079e84eba0a3a5e3133e3700ce2da17db6d2dcc061e23648cf20ac5471653cf`. The committed neutral derivative
measures 19,466,069 bytes and removes authoring-environment paths, project/task identifiers, and source
comments that were not needed to decode the authored scene. The authored geometry, material networks,
variant selections, skeleton, animation, and eight JPEG payloads are retained.

## Reference relationships

- `RobotArm2.usda` authors `prepend references = @./RobotArm2/RobotArm.usda@</RobotArm>`.
  The reference is resolved relative to the wrapper layer and grafted beneath the authored
  `RobotArm2` prim.
...
`RobotArm/industrial_robot_arm.obj` and `RobotArm/industrial_robot_arm.mtl` retain their
embedded Blender attribution headers. The color texture has no embedded attribution metadata.

`RobotArm2.usda` contains no embedded attribution metadata beyond its neutral USDA comments.
...
`seahorse_anim_mtl_variant.usdz` retains the supplied eight JPEG payloads and authored USDC scene
data. Its committed root crate retains the observed attribution
`Copyright (c) 2025 Apple Inc. All rights reserved.` while removing authoring-environment paths,
project/task identifiers, UUIDs, and non-essential generator metadata. The archive-local paths were
audited and contain only the nine neutral entry names listed above.
...
- `RobotArm2.usda` is an unmodified byte-for-byte wrapper from the provided snapshot. Its
  comments, metadata, and authored relative reference were audited for private identifiers;
  none were found.
...
- `seahorse_anim_mtl_variant.usdz` is a redacted derivative: authoring-environment paths,
  project/task identifiers, UUIDs, and non-essential generator metadata were removed from the
  embedded USDC root. Authored geometry, material networks, variant selections, skeleton,
  animation, attribution, and eight JPEG payloads are retained; committed bytes and hashes reflect
...
- `RobotArm.usda`, `RobotArm/industrial_robot_arm.obj`, `RobotArm/industrial_robot_arm.mtl`, and
  `RobotArm/Robot_Arm_Color.png` are unmodified byte-for-byte copies of the provided source
  snapshot. A confidentiality audit found no authoring-environment identifiers in their comments,
  metadata, or paths, so no redaction was applied.
...
- `Plane.usda` is an internally-authored fixture (not partner-sourced); its `faceVertexIndices`
  winding was corrected from `[0, 1, 2, 3]` to `[0, 3, 2, 1]`. The original order geometrically
  wound clockwise as seen from the authored `(0, 1, 0)` normal's side, which is only exposed once
  the mesh is tessellated (the default unauthored `catmullClark` subdivision discards constant
  authored normals and recomputes them from triangle winding, per `ResolveSubdivisionScheme`'s
  existing polygon-mesh-intent contract). The corrected winding is geometrically consistent with
  the authored normal in both the non-subdivided and tessellated paths; committed bytes and hash
...
> **TODO — Robot Arm licensing:** Verify redistribution permission and attribution
> requirements for the wrapper, OBJ, MTL, and referenced color PNG before any public release.
...
- `RobotArm.usda` references `./RobotArm/industrial_robot_arm.obj`; that OBJ requests only
  `industrial_robot_arm.mtl`, and the active MTL requests only `Robot_Arm_Color.png`.
  The unused duplicate/orphan `industrial robot arm.mtl` is intentionally not copied or
  requested. No GLB or binary USD sidecar belongs to this wrapper.
- `seahorse_anim_mtl_variant.usdz` contains the embedded USDC root and eight JPEG entries listed in
  the embedded-entry manifest. The loader selects the USDC entry by `PXR-USDC` magic and resolves
  normalized archive-local JPEG paths to data URIs without a global file registry.
...
> **TODO — Seahorse USDZ licensing:** Verify redistribution permission and attribution requirements
> for `seahorse_anim_mtl_variant.usdz`, its embedded USDC, and all eight JPEG entries before any
> public release. Retain the Apple attribution above when licensing is confirmed.
