...
/**
 * @experimental
 * OpenUSD scene loader plugin for single-layer USDA text and USDC crate (`.usd`/`.usda`).
 *
 * Input is selected by content rather than by extension: USDA text is parsed, while binary crate
 * (`PXR-USDC`) bytes are decoded by the bounded crate reader, while USDZ package (ZIP) bytes are
 * rejected with a typed {@link UsdUnsupportedFormatError}. The loader is split into a USD *resolution
 * layer* (parsing, single-layer validation and stage/time evaluation, producing a fully-resolved
