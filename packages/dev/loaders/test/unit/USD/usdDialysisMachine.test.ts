 ...
 * @returns an external asset handler for the Dialysis Machine corpus fixture
 */
...
describe("USD RuntimeCorpus - Dialysis Machine", () => {
    ...
    it("has representative authored MTL values on Chrome_metal, White_Plastic_Front, and Trans_Plastic_1", () => {
        ...
        expect(chrome.specularColor.b).toBeCloseTo(0.9, 2);
...
def Xform "DialysisMachine"
{
    ...
    it("logs a structured diagnostic when no external asset handler is configured", async () => {
        const engine = new NullEngine();
        const scene = new Scene(engine);
        ...
        try {
            ...
            expect(result.transformNodes.some((node) => node.name === "DialysisMachine")).toBe(true);
            ...
            expect(diagnostic).toContain("/DialysisMachine/Asset.assetInfo:source");
        } finally {
            scene.dispose();
            engine.dispose();
        }
