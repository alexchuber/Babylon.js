...
/** Deduplicates equivalent textures in a Universal payload. */
export class DeduplicateTexturesBlock extends NodeAssetBlock {
    /** The class name, used for identification and safe under minification. */
    public static override ClassName = "DeduplicateTexturesBlock";

    /** The Universal payload to process. */
    public readonly input: NodeAssetConnectionPoint;
    /** The processed Universal payload. */
    public readonly output: NodeAssetConnectionPoint;
    /** Whether otherwise equivalent textures with different names remain separate. */
    public keepUniqueNames = false;

    /**
     * Creates a Deduplicate Textures block.
    ...
    /** Deduplicates textures and forwards the same Universal payload. */
    public override async _buildBlockAsync(): Promise<void> {
        const { dedup } = await import("@gltf-transform/functions");
        await ApplyUniversalOperatorTransformsAsync(this, dedup({ propertyTypes: [PropertyType.TEXTURE], keepUniqueNames: this.keepUniqueNames }));
    }

    /**
     * Serializes the texture deduplication configuration.
    ...
    /**
     * Restores the texture deduplication configuration.
...
RegisterBlock(DeduplicateTexturesBlock.ClassName, (name, nodeAsset) => new DeduplicateTexturesBlock(name, nodeAsset));
