...
describe("Babylon glTF Serializer", () => {
    ...
    describe("#GLTF Serializer", () => {
        ...
        it("should export camera rotation animations, left-handed", async () => {
            const assertionData = await page.evaluate(async () => {
                const camera1 = new BABYLON.FreeCamera("camera1", new BABYLON.Vector3(0, 2, -4));
                camera1.rotationQuaternion = camera1.rotation.toQuaternion();

                const keys = [];
                keys.push({
                    frame: 0,
                    value: camera1.rotationQuaternion.clone(),
                });
                keys.push({
                    frame: 1,
                    value: BABYLON.Quaternion.FromEulerAngles(0, Math.PI / 4, 0).multiply(keys[0].value),
                });
                keys.push({
                    frame: 2,
                    value: BABYLON.Quaternion.FromEulerAngles(-Math.PI / 4, 0, 0).multiply(keys[1].value),
                });
                ...
                animationGroup.addTargetedAnimation(animation, camera1);

                const glbData = await BABYLON.GLTF2Export.GLBAsync(window.scene!, "test");
                const glTFData = await BABYLON.GLTF2Export.GLTFAsync(window.scene!, "test");
                const blob = glbData.files["test.glb"];
                ...
                const container = await BABYLON.SceneLoader.LoadAssetContainerAsync("file:", file);
                container.addAllToScene();

                const finalKeys = [];
                for (let i = 0; i < 2; i++) {
                    const animation = window.scene!.animationGroups[i].targetedAnimations[0].animation;
                    finalKeys.push(
                        animation
                            .getKeys()
                ...
                return {
                    firstKeys: finalKeys[0],
                    secondKeys: finalKeys[1],
                    glTF: JSON.parse(glTFData.files["test.gltf"] as string),
                };
            });
            expect(assertionData.glTF).toBeDefined();
            expect(assertionData.firstKeys).toEqual(assertionData.secondKeys);
        });
    });
